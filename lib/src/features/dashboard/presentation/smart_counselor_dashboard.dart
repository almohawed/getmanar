...
// Vision 2030 Colors - Purple/Teal Theme for Counseling
const _kPrimary = Color(0xFF6A1B9A);
const _kSecondary = Color(0xFF8E24AA);
...
class SmartCounselorDashboard extends ConsumerStatefulWidget {
  const SmartCounselorDashboard({super.key});
  @override
  ConsumerState<SmartCounselorDashboard> createState() => _SmartCounselorDashboardState();
}

class _SmartCounselorDashboardState extends ConsumerState<SmartCounselorDashboard> with TickerProviderStateMixin {
  ...
  Future<Map<String, int>> _getCounselorStats(String schoolId) async {
    ...
    try {
      ...
      final sessionsSnap = await FirebaseFirestore.instance.collection('Schools').doc(schoolId).collection('CounselingSessions').where('date', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfMonth)).get();
      final followUpsSnap = await FirebaseFirestore.instance.collection('Schools').doc(schoolId).collection('StudentFollowUps').where('status', isEqualTo: 'active').get();
      
      int activeCases = casesSnap.docs.where((d) => d.data()['status'] != 'closed').length;
      int closedCases = casesSnap.docs.where((d) => d.data()['status'] == 'closed' && (d.data()['closedAt'] as Timestamp?)?.toDate().isAfter(startOfMonth) == true).length;
      int scheduledSessions = sessionsSnap.docs.where((d) => (d.data()['date'] as Timestamp?)?.toDate().isAfter(now) == true).length;
      int followUps = followUpsSnap.docs.length;
      
      return {'activeCases': activeCases, 'closedCases': closedCases, 'scheduledSessions': scheduledSessions, 'followUps': followUps};
    } catch (e) {
      return {};
    }
  }
    ...
    return SliverAppBar(
      ...
      flexibleSpace: FlexibleSpaceBar(
        ...
        background: Container(
          decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF4A148C), Color(0xFF6A1B9A), Color(0xFF8E24AA), Color(0xFF00897B)], begin: Alignment.topRight, end: Alignment.bottomLeft)),
            ...
            SafeArea(child: Padding(padding: EdgeInsets.fromLTRB(16.w, 50.h, 16.w, 16.h), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('مركز التوجيه والإرشاد الطلابي', style: GoogleFonts.cairo(color: Colors.white, fontSize: 16.sp, fontWeight: FontWeight.w800, letterSpacing: 0.5)),
                  Text('نظام ذكي متكامل للرعاية النفسية', style: GoogleFonts.cairo(color: Colors.white.withOpacity(0.85), fontSize: 11.sp)),
                ])),
              ]),
        ...
        title: Text('التوجيه الطلابي', style: GoogleFonts.cairo(color: Colors.white, fontSize: 15.sp, fontWeight: FontWeight.w700)),
  ...
  Widget _buildKPICards(String schoolId) {
    return FutureBuilder<Map<String, int>>(
      future: _getCounselorStats(schoolId),
      builder: (context, snap) {
        final stats = snap.data ?? {};
        final activeCases = stats['activeCases'] ?? 0;
        final closedCases = stats['closedCases'] ?? 0;
        final scheduledSessions = stats['scheduledSessions'] ?? 0;
        final followUps = stats['followUps'] ?? 0;
        
        return Column(children: [
          Row(children: [
            Expanded(child: _buildKPICard('الحالات النشطة', activeCases.toString(), Icons.folder_open_rounded, _kWarning, 'قيد المتابعة')),
            SizedBox(width: 12.w),
            Expanded(child: _buildKPICard('الحالات المغلقة', closedCases.toString(), Icons.check_circle_rounded, _kSuccess, 'هذا الشهر')),
          ]),
          SizedBox(height: 12.h),
          Row(children: [
            Expanded(child: _buildKPICard('الجلسات المجدولة', scheduledSessions.toString(), Icons.event_rounded, _kInfo, 'قادمة')),
            SizedBox(width: 12.w),
            Expanded(child: _buildKPICard('طلاب المتابعة', followUps.toString(), Icons.people_rounded, _kAccent, 'نشط')),
          ]),
        ]);
      },
    );
  ...
  Widget _buildSmartInsights(String schoolId) {
    return Container(
      ...
      decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF6A1B9A), Color(0xFF8E24AA)], begin: Alignment.topRight, end: Alignment.bottomLeft), borderRadius: BorderRadius.circular(24.r), boxShadow: [BoxShadow(color: _kPrimary.withOpacity(0.3), blurRadius: 20, offset: const Offset(0, 10))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(height: 16.h),
        _buildInsightItem(Icons.trending_up_rounded, Colors.green.shade300, 'تحسن ملحوظ في معدل إغلاق الحالات بنسبة 20% مقارنة بالشهر الماضي'),
        SizedBox(height: 10.h),
        _buildInsightItem(Icons.calendar_today_rounded, Colors.blue.shade300, 'يُنصح بجدولة جلسات متابعة للحالات النشطة خلال الأسبوع القادم'),
        SizedBox(height: 10.h),
        _buildInsightItem(Icons.groups_rounded, Colors.amber.shade300, 'التركيز على الطلاب ذوي الحالات المتكررة لتطوير خطط تعديل سلوك'),
      ]),
    );
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          SizedBox(width: 10.w),
          Text('الجلسات الإرشادية - آخر 7 أيام', style: GoogleFonts.cairo(fontSize: 15.sp, fontWeight: FontWeight.w700, color: _kPrimary)),
        ]),
  ...
  Widget _buildMockChart() {
    final data = [5.0, 8.0, 6.0, 10.0, 7.0, 9.0, 12.0];
      ...
      BarChartData(
        ...
        barGroups: List.generate(data.length, (i) {
          ...
          return BarChartGroupData(x: i, barRods: [
            BarChartRodData(toY: data[i], color: isToday ? _kAccent : (data[i] > 8 ? _kSuccess : _kInfo), width: 20.w, borderRadius: BorderRadius.only(topLeft: Radius.circular(6.r), topRight: Radius.circular(6.r)), backDrawRodData: BackgroundBarChartRodData(show: true, toY: maxVal, color: Colors.grey.shade100)),
          ]);
        }),
        barTouchData: BarTouchData(touchTooltipData: BarTouchTooltipData(getTooltipItem: (group, groupIndex, rod, rodIndex) {
          return BarTooltipItem(' جلسة', GoogleFonts.cairo(color: Colors.white, fontSize: 11.sp, fontWeight: FontWeight.w600));
        })),
      ),
    );
  }
  ...
  Widget _buildActionButtons(BuildContext context) {
    return Column(children: [
      Row(children: [
        Expanded(child: _buildActionButton(context, 'إضافة حالة', Icons.add_circle_rounded, _kPrimary, '/add-student-case')),
        SizedBox(width: 12.w),
        Expanded(child: _buildActionButton(context, 'جلسة إرشادية', Icons.event_note_rounded, _kAccent, '/add-counseling-session')),
      ]),
      SizedBox(height: 12.h),
      Row(children: [
        Expanded(child: _buildActionButton(context, 'الحالات النشطة', Icons.folder_open_rounded, _kWarning, '/counselor/active-cases')),
        SizedBox(width: 12.w),
        Expanded(child: _buildActionButton(context, 'خطط التعديل', Icons.assignment_rounded, _kInfo, '/counselor/active-plans')),
      ]),
      SizedBox(height: 12.h),
      Row(children: [
        Expanded(child: _buildActionButton(context, 'الحالات الصحية', Icons.health_and_safety_rounded, Colors.cyan, '/counselor/health-cases')),
        SizedBox(width: 12.w),
        Expanded(child: _buildActionButton(context, 'التقارير', Icons.assessment_rounded, Colors.deepPurple, '/counselor/comprehensive-report')),
      ]),
    ]);
  }
